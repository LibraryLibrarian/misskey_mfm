...
class MathBlockParser {
  ...
  Parser<MfmNode> build() {
    ...
    final formulaChar = ((lineBreak.optional() & close).not() & any()).pick(1);
    ...
    //
    // 構造: [前の改行?, lineBegin, \[, 改行?, 数式, 改行?, \], lineEnd, 後の改行?]
    // mfm.js と同様に lineBegin の前で境界改行を消費する。
    final startPart = seq4(
      lineBreak.optional(),
      lineBegin(),
      open,
      lineBreak.optional(),
    );
    final endPart = seq3(lineBreak.optional(), close, lineEnd());

    return seq4(startPart, formula, endPart, lineBreak.optional()).map4((
      start,
      formulaStr,
      end,
      trailingNewline,
